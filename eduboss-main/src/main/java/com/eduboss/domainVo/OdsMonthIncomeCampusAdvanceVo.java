package com.eduboss.domainVo;

import java.math.BigDecimal;

public class OdsMonthIncomeCampusAdvanceVo {

	private String groupName;
	private String brenchName;
	private String campusName;
	private BigDecimal smallClassRealAmount;
	private BigDecimal smallClassPromotionAmount;
	private BigDecimal smallClassRealWashAmount;
	private BigDecimal smallClassPromotionWashAmount;
	private BigDecimal smallClassTotalAmount;
	private BigDecimal liveNewRealAmount;
	private BigDecimal liveNewRealDivide;
	private BigDecimal liveRenewRealAmount;
	private BigDecimal liveRenewRealDivide;
	private BigDecimal liveTotalAmount;
	private BigDecimal liveTotalDivide;
	private BigDecimal lectureRealAmount;
	private BigDecimal lecturePromotionAmount;
	private BigDecimal lectureRealWashAmount;
	private BigDecimal lecturePromotionWashAmount;
	private BigDecimal lectureTotalAmount;
	private BigDecimal otherRealAmount;
	private BigDecimal otherPromotionAmount;
	private BigDecimal otherRealWashAmount;
	private BigDecimal otherPromotionWashAmount;
	private BigDecimal otherTotalAmount;
	private BigDecimal isNormalRealAmount;
	private BigDecimal isNormalPromotionAmount;
	private BigDecimal isNormalHistoryWashAmount;
	private BigDecimal isNormalTotalAmount;
	private BigDecimal realTotalAmount;
	private BigDecimal promotionTotalAmount;
	private BigDecimal realWashTotalAmount;
	private BigDecimal promotionWashTotalAmount;
	private BigDecimal historyWashTotalAmount;
	private BigDecimal totalAmount;
	private BigDecimal adjustTotalAmount;
	private BigDecimal afterAdjustTotalAmount;
	private String edivenceRemark;
	private String edivenceMonth;
	private String mappingDate;
	private String evidenceAuditStatusName;


	public String getGroupName() {
		return groupName;
	}

	public void setGroupName(String groupName) {
		this.groupName = groupName;
	}

	public String getBrenchName() {
		return brenchName;
	}

	public void setBrenchName(String brenchName) {
		this.brenchName = brenchName;
	}

	public String getCampusName() {
		return campusName;
	}

	public void setCampusName(String campusName) {
		this.campusName = campusName;
	}

	public BigDecimal getSmallClassRealAmount() {
		return smallClassRealAmount;
	}

	public void setSmallClassRealAmount(BigDecimal smallClassRealAmount) {
		this.smallClassRealAmount = smallClassRealAmount;
	}

	public BigDecimal getSmallClassPromotionAmount() {
		return smallClassPromotionAmount;
	}

	public void setSmallClassPromotionAmount(BigDecimal smallClassPromotionAmount) {
		this.smallClassPromotionAmount = smallClassPromotionAmount;
	}

	public BigDecimal getSmallClassRealWashAmount() {
		return smallClassRealWashAmount;
	}

	public void setSmallClassRealWashAmount(BigDecimal smallClassRealWashAmount) {
		this.smallClassRealWashAmount = smallClassRealWashAmount;
	}

	public BigDecimal getSmallClassPromotionWashAmount() {
		return smallClassPromotionWashAmount;
	}

	public void setSmallClassPromotionWashAmount(BigDecimal smallClassPromotionWashAmount) {
		this.smallClassPromotionWashAmount = smallClassPromotionWashAmount;
	}

	public BigDecimal getSmallClassTotalAmount() {
		return smallClassTotalAmount;
	}

	public void setSmallClassTotalAmount(BigDecimal smallClassTotalAmount) {
		this.smallClassTotalAmount = smallClassTotalAmount;
	}

	public BigDecimal getLectureRealAmount() {
		return lectureRealAmount;
	}

	public void setLectureRealAmount(BigDecimal lectureRealAmount) {
		this.lectureRealAmount = lectureRealAmount;
	}

	public BigDecimal getLecturePromotionAmount() {
		return lecturePromotionAmount;
	}

	public void setLecturePromotionAmount(BigDecimal lecturePromotionAmount) {
		this.lecturePromotionAmount = lecturePromotionAmount;
	}

	public BigDecimal getLectureRealWashAmount() {
		return lectureRealWashAmount;
	}

	public void setLectureRealWashAmount(BigDecimal lectureRealWashAmount) {
		this.lectureRealWashAmount = lectureRealWashAmount;
	}

	public BigDecimal getLecturePromotionWashAmount() {
		return lecturePromotionWashAmount;
	}

	public void setLecturePromotionWashAmount(BigDecimal lecturePromotionWashAmount) {
		this.lecturePromotionWashAmount = lecturePromotionWashAmount;
	}

	public BigDecimal getLectureTotalAmount() {
		return lectureTotalAmount;
	}

	public void setLectureTotalAmount(BigDecimal lectureTotalAmount) {
		this.lectureTotalAmount = lectureTotalAmount;
	}

	public BigDecimal getOtherRealAmount() {
		return otherRealAmount;
	}

	public void setOtherRealAmount(BigDecimal otherRealAmount) {
		this.otherRealAmount = otherRealAmount;
	}

	public BigDecimal getOtherPromotionAmount() {
		return otherPromotionAmount;
	}

	public void setOtherPromotionAmount(BigDecimal otherPromotionAmount) {
		this.otherPromotionAmount = otherPromotionAmount;
	}

	public BigDecimal getOtherRealWashAmount() {
		return otherRealWashAmount;
	}

	public void setOtherRealWashAmount(BigDecimal otherRealWashAmount) {
		this.otherRealWashAmount = otherRealWashAmount;
	}

	public BigDecimal getOtherPromotionWashAmount() {
		return otherPromotionWashAmount;
	}

	public void setOtherPromotionWashAmount(BigDecimal otherPromotionWashAmount) {
		this.otherPromotionWashAmount = otherPromotionWashAmount;
	}

	public BigDecimal getOtherTotalAmount() {
		return otherTotalAmount;
	}

	public void setOtherTotalAmount(BigDecimal otherTotalAmount) {
		this.otherTotalAmount = otherTotalAmount;
	}

	public BigDecimal getIsNormalRealAmount() {
		return isNormalRealAmount;
	}

	public void setIsNormalRealAmount(BigDecimal isNormalRealAmount) {
		this.isNormalRealAmount = isNormalRealAmount;
	}

	public BigDecimal getIsNormalPromotionAmount() {
		return isNormalPromotionAmount;
	}

	public void setIsNormalPromotionAmount(BigDecimal isNormalPromotionAmount) {
		this.isNormalPromotionAmount = isNormalPromotionAmount;
	}

	public BigDecimal getIsNormalHistoryWashAmount() {
		return isNormalHistoryWashAmount;
	}

	public void setIsNormalHistoryWashAmount(BigDecimal isNormalHistoryWashAmount) {
		this.isNormalHistoryWashAmount = isNormalHistoryWashAmount;
	}

	public BigDecimal getIsNormalTotalAmount() {
		return isNormalTotalAmount;
	}

	public void setIsNormalTotalAmount(BigDecimal isNormalTotalAmount) {
		this.isNormalTotalAmount = isNormalTotalAmount;
	}

	public BigDecimal getRealTotalAmount() {
		return realTotalAmount;
	}

	public void setRealTotalAmount(BigDecimal realTotalAmount) {
		this.realTotalAmount = realTotalAmount;
	}

	public BigDecimal getPromotionTotalAmount() {
		return promotionTotalAmount;
	}

	public void setPromotionTotalAmount(BigDecimal promotionTotalAmount) {
		this.promotionTotalAmount = promotionTotalAmount;
	}

	public BigDecimal getRealWashTotalAmount() {
		return realWashTotalAmount;
	}

	public void setRealWashTotalAmount(BigDecimal realWashTotalAmount) {
		this.realWashTotalAmount = realWashTotalAmount;
	}

	public BigDecimal getPromotionWashTotalAmount() {
		return promotionWashTotalAmount;
	}

	public void setPromotionWashTotalAmount(BigDecimal promotionWashTotalAmount) {
		this.promotionWashTotalAmount = promotionWashTotalAmount;
	}

	public BigDecimal getHistoryWashTotalAmount() {
		return historyWashTotalAmount;
	}

	public void setHistoryWashTotalAmount(BigDecimal historyWashTotalAmount) {
		this.historyWashTotalAmount = historyWashTotalAmount;
	}

	public BigDecimal getTotalAmount() {
		return totalAmount;
	}

	public void setTotalAmount(BigDecimal totalAmount) {
		this.totalAmount = totalAmount;
	}

	public BigDecimal getAdjustTotalAmount() {
		return adjustTotalAmount;
	}

	public void setAdjustTotalAmount(BigDecimal adjustTotalAmount) {
		this.adjustTotalAmount = adjustTotalAmount;
	}

	public BigDecimal getAfterAdjustTotalAmount() {
		return afterAdjustTotalAmount;
	}

	public void setAfterAdjustTotalAmount(BigDecimal afterAdjustTotalAmount) {
		this.afterAdjustTotalAmount = afterAdjustTotalAmount;
	}

	public String getEdivenceRemark() {
		return edivenceRemark;
	}

	public void setEdivenceRemark(String edivenceRemark) {
		this.edivenceRemark = edivenceRemark;
	}

	public String getEdivenceMonth() {
		return edivenceMonth;
	}

	public void setEdivenceMonth(String edivenceMonth) {
		this.edivenceMonth = edivenceMonth;
	}

	public String getMappingDate() {
		return mappingDate;
	}

	public void setMappingDate(String mappingDate) {
		this.mappingDate = mappingDate;
	}

	public String getEvidenceAuditStatusName() {
		return evidenceAuditStatusName;
	}

	public void setEvidenceAuditStatusName(String evidenceAuditStatusName) {
		this.evidenceAuditStatusName = evidenceAuditStatusName;
	}

	public BigDecimal getLiveNewRealAmount() {
		return liveNewRealAmount;
	}

	public void setLiveNewRealAmount(BigDecimal liveNewRealAmount) {
		this.liveNewRealAmount = liveNewRealAmount;
	}

	public BigDecimal getLiveNewRealDivide() {
		return liveNewRealDivide;
	}

	public void setLiveNewRealDivide(BigDecimal liveNewRealDivide) {
		this.liveNewRealDivide = liveNewRealDivide;
	}

	public BigDecimal getLiveRenewRealAmount() {
		return liveRenewRealAmount;
	}

	public void setLiveRenewRealAmount(BigDecimal liveRenewRealAmount) {
		this.liveRenewRealAmount = liveRenewRealAmount;
	}

	public BigDecimal getLiveRenewRealDivide() {
		return liveRenewRealDivide;
	}

	public void setLiveRenewRealDivide(BigDecimal liveRenewRealDivide) {
		this.liveRenewRealDivide = liveRenewRealDivide;
	}

	public BigDecimal getLiveTotalAmount() {
		return liveTotalAmount;
	}

	public void setLiveTotalAmount(BigDecimal liveTotalAmount) {
		this.liveTotalAmount = liveTotalAmount;
	}

	public BigDecimal getLiveTotalDivide() {
		return liveTotalDivide;
	}

	public void setLiveTotalDivide(BigDecimal liveTotalDivide) {
		this.liveTotalDivide = liveTotalDivide;
	}
	
	
}
