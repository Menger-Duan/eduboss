...
@Component
@Scope("prototype")
public class SendJoinMiniCourseMsgThread implements Runnable {
    ...
    private static final Log log = LogFactory.getLog(SendJoinMiniCourseMsgThread.class);

    public SendJoinMiniCourseMsgThread() {
    }

    public SendJoinMiniCourseMsgThread(String miniClassCourseId, String studentId, String currentUserId) {
    ...
    public void run() {
        log.debug("进入方法！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！");
        SystemMessageManage sysMsg = sysMsgManageService.findSystemMessageManageByMsgNo(MsgNo.M11);
        if (sysMsg != null) {
            String sql = sysMsg.getLogicSql();
            if (StringUtil.isNotBlank(sql)){
            	sql = sql.replace("${STUDENT_ID}", studentId);
                sql += " AND mcc.MINI_CLASS_COURSE_ID = '"+ miniClassCourseId + "' ";
//                sql += " AND mcsa.STUDENT_ID =  '" + studentId + "' ";
...
//                                    String result = MessageUtil.readContentFromGet(phones, tempSmsMsg);
                                String miniClassName = (String) map.get("miniClassName");
                                miniClassName = miniClassName.length() > 15 ? miniClassName.substring(0, 12) + "..." : miniClassName;
                                if (AliyunSmsUtil.sendSms(AliyunSmsUtil.SIGN_NAME, "SMS_48010227", phones, 
            							"{\"miniClassName\":\""+  miniClassName + "\",\"courseDate\":\"" + map.get("courseDate") + "\","
            								+ "\"courseTime\":\"" + map.get("courseTime") + "\","
            								+ "\"studentName\":\"" + map.get("studentName") + "\"}")) {
                                	if (StringUtil.isNotBlank(teacherPhone)) {
                                		SentRecord record = sentRecordService.saveSentRecord(sysMsg, "SMS", tempSendMsg, sendDetailedMsg, currentUserId, teacherMsgRecipient, teacherPhone, MsgNo.M11);
                                	...
                                	if (StringUtil.isNotBlank(studyManagerPhone)) {
                                		SentRecord record = sentRecordService.saveSentRecord(sysMsg, "SMS", tempSendMsg, sendDetailedMsg, currentUserId, studyManagerMsgRecipient, studyManagerPhone, MsgNo.M11);
                                		record.setPushMsgType(PushMsgType.SYSTEM_MESSAGE);
                                		record.setSysMsgType(SysMsgType.MINI_CLASS);
                                		record.setDetailId(miniClassCourseId);
                                		sentRecordService.saveOrUpdateSentRecord(record);
                                ...
                                log.error("系统信息，编号：" +MsgNo.M11.getValue() + "课程：" + miniClassCourseId + "的老师和学管没有配置手机号码!");
                            }
                        }
                        if (sendType.indexOf("SYS_MSG") > -1) {   
                            ...
                            if (StringUtil.isNotBlank(teacherPhone)) {
                            	SentRecord record = sentRecordService.saveSentRecord(sysMsg, "SYS_MSG", tempSendMsg, sendDetailedMsg, currentUserId, teacherMsgRecipient, teacherPhone, MsgNo.M11);
                            	record.setPushMsgType(PushMsgType.SYSTEM_MESSAGE);
                            	record.setSysMsgType(SysMsgType.MINI_CLASS);
                            	record.setDetailId(miniClassCourseId);
                            	sentRecordService.saveOrUpdateSentRecord(record);
                            	mobilePushMsgService.pushMsg(teacher, tempSendMsg, record.getId(), PushMsgType.SYSTEM_MESSAGE.getValue(), DateTools.getCurrentDateTime(), SysMsgType.MINI_CLASS, miniClassCourseId, msgTypeName);
                            }
                            if (StringUtil.isNotBlank(studyManagerPhone)) {
                            	SentRecord record = sentRecordService.saveSentRecord(sysMsg, "SYS_MSG", tempSendMsg, sendDetailedMsg, currentUserId, studyManagerMsgRecipient, studyManagerPhone, MsgNo.M11);
                            	record.setPushMsgType(PushMsgType.SYSTEM_MESSAGE);
                            	record.setSysMsgType(SysMsgType.MINI_CLASS);
                            	record.setDetailId(miniClassCourseId);
                            	sentRecordService.saveOrUpdateSentRecord(record);
                            	mobilePushMsgService.pushMsg(studyManager, tempSendMsg, record.getId(), PushMsgType.SYSTEM_MESSAGE.getValue(), DateTools.getCurrentDateTime(), SysMsgType.MINI_CLASS, miniClassCourseId, msgTypeName);
                            }
                        }
                    }
                }else {
                    log.error("系统信息，编号：" +MsgNo.M11.getValue() + "没有配置发送类型!");
                }
            } else {
                log.error("系统信息，编号：" +MsgNo.M11.getValue() + "没有配置sql!");
            }
        }
