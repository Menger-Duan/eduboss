...
public class SendOtmSysMsgThread implements Runnable {
	...
	public SendOtmSysMsgThread(){}
	public SendOtmSysMsgThread(String otmCourseId, String currentUserId, String transactionId){
	...
	@Override
	public void run(){
		SystemMessageManage sysMsg = sysMsgManageService.findSystemMessageManageByMsgNo(MsgNo.M2);
		if (sysMsg != null) {
				...
				if (StringUtil.isNotBlank(sendType)) {
					...
					for (Map<String, Object> map : list) {
							...
							if (StringUtil.isNotBlank(phoneNumBer)) {
								try {
									...
									if (Integer.parseInt(result) > 0) {
										record = sentRecordService.saveSentRecord(sysMsg, "SMS", tempSendMsg,sendDetailedMsg, currentUserId, msgRecipient, phoneNumBer, MsgNo.M2);
						...
						if (sendType.indexOf("SYS_MSG") > -1) {
							SentRecord record = null;								
							record = sentRecordService.saveSentRecord(sysMsg, "SYS_MSG", tempSendMsg,sendDetailedMsg, currentUserId, msgRecipient, phoneNumBer,MsgNo.M2);
							record.setPushMsgType(PushMsgType.SYSTEM_MESSAGE);
							record.setSysMsgType(SysMsgType.OTM_CLASS);
							record.setDetailId(otmCourseId);
							sentRecordService.saveOrUpdateSentRecord(record);
