...
@Component  
@Scope("prototype")
public class SendRefundSysMsgThread implements Runnable {
	...
	private MsgNo msgNo;
	public SendRefundSysMsgThread(){}
	public SendRefundSysMsgThread(String refundWorkflowId, String currentUserId, MsgNo msgNo) {
	...
	public void run() {
		log.debug("进入方法！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！");
		SystemMessageManage sysMsg = sysMsgManageService.findSystemMessageManageByMsgNo(msgNo);
		if (sysMsg != null) {
			String sql = sysMsg.getLogicSql();
			if (StringUtil.isNotBlank(sql)) {
				sql += " AND rw.ID = '"+ refundWorkflowId + "' ";
						...
						if (sendType.indexOf("SMS") > -1) {
							...
							if (StringUtil.isNotBlank(phoneNumBer)) {
								try {
									...
									sendDetailedMsg = StringUtil.replacePlaceholder(msgContent, map, "${", "}");
									...
									if (Integer.parseInt(result) > 0) {
										record = sentRecordService.saveSentRecord(sysMsg, "SMS", tempSendMsg,sendDetailedMsg, currentUserId, msgRecipient, phoneNumBer,msgNo);
										record.setPushMsgType(PushMsgType.SYSTEM_MESSAGE);
										record.setSysMsgType(SysMsgType.REFUND_AUDIT);
								...
								log.error("系统信息，编号：" +msgNo.getValue() + "审批申请：" + refundWorkflowId + "的老师没有配置手机号码!");
							}
						} 
						if (sendType.indexOf("SYS_MSG") > -1) {
							record = sentRecordService.saveSentRecord(sysMsg, "SYS_MSG", tempSendMsg,sendDetailedMsg, currentUserId, msgRecipient, phoneNumBer,msgNo);
							record.setPushMsgType(PushMsgType.SYSTEM_MESSAGE);
							record.setSysMsgType(SysMsgType.REFUND_AUDIT);
							sentRecordService.saveOrUpdateSentRecord(record);
							...
							mobilePushMsgService.pushMsg(user, tempSendMsg, record.getId(), PushMsgType.SYSTEM_MESSAGE.getValue(), DateTools.getCurrentDateTime(), SysMsgType.REFUND_AUDIT, "", msgTypeName);
						}
					}
				} else {
					log.error("系统信息，编号：" +msgNo.getValue() + "没有配置发送类型!");
				}
			} else {
				log.error("系统信息，编号：" +msgNo.getValue() + "没有配置sql!");
			}
		}
	}
}
