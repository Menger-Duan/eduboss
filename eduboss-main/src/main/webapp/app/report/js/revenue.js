...
function loadFinanceAnalyze(token, startDate, endDate, basicOperationQueryLevelType) {
	$.ajax({
        ...
        url: "/eduboss/MobileInterface/getIncomingAnalyze.do?token=" + token 
        	+ "&startDate=" + startDate + "&endDate=" + endDate + "&basicOperationQueryLevelType=" + basicOperationQueryLevelType,
        ...
        success: function(res){
        	...
        	$.each(res.rows, function(n, item) {
        		var brenchId = item.brenchId;
        		var brenchName = item.brenchName;
	            ...
	            if ('BRENCH' == basicOperationQueryLevelType) {
	            	tr +=  '<td class="revenueTdR2"><div class="revenueTdR2" style="overflow:hidden;">' + item.campusName + '</div></td>';
	            } else {
	            	 tr +=  '<td class="revenueTdR2"><div class="revenueTdR2" style="overflow:hidden;"><a href="revenueBranch.html?brenchId=' + brenchId + '&brenchName=' + encodeURI(brenchName) + '">' + brenchName + '</a></div></td>';
	            }
